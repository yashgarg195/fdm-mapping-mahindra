...
import pandas as pd
from ui.theme import NEUTRAL, section_header
...
def _download_row(label, buf, filename, description=""):
    ...
    with col_desc:
        st.markdown(
            ...
            f"<span style='font-weight:700; color:{NEUTRAL['ink']}; font-size:0.95rem;'>{label}</span>"
            + (f"<br><span style='color:{NEUTRAL['muted']}; font-size:0.8rem;'>{description}</span>" if description else "")
            + "</div>",
    ...
    st.markdown(
        f"<div style='border-bottom:1px solid {NEUTRAL['line']}; margin:4px 0 8px 0;'></div>",
        unsafe_allow_html=True,
    )
...
def render_export_tab(unified_df, backlog_df, nomination_df, duplicate_df, audit_log, filters, kpis, stats):
    ...
    st.markdown(
        f"<div style='background:#ffffff; border:1px solid {NEUTRAL['line']}; border-radius:12px; padding:14px 18px; "
        f"display:flex; gap:32px; flex-wrap:wrap; margin-bottom:20px;'>"
        f"<div><span style='color:{NEUTRAL['muted']}; font-size:0.78rem;'>REPORT GENERATED</span><br>"
        f"<span style='font-weight:700; font-size:0.92rem;'>{now}</span></div>"
        f"<div><span style='color:{NEUTRAL['muted']}; font-size:0.78rem;'>TOTAL RECORDS</span><br>"
        f"<span style='font-weight:700; font-size:0.92rem;'>{row_count:,}</span></div>"
        f"<div><span style='color:{NEUTRAL['muted']}; font-size:0.78rem;'>FILTERS APPLIED</span><br>"
        f"<span style='font-weight:700; font-size:0.92rem; color:{NEUTRAL['info'] if active_filters else NEUTRAL['accent']};'>"
        f"{'Yes — ' + ', '.join(active_filters.keys()) if active_filters else 'None (all data)'}</span></div>"
        f"<div><span style='color:{NEUTRAL['muted']}; font-size:0.78rem;'>UNIQUE MANPOWER</span><br>"
        f"<span style='font-weight:700; font-size:0.92rem;'>{kpis.get('total_manpower', 0):,}</span></div>"
        f"<div><span style='color:{NEUTRAL['muted']}; font-size:0.78rem;'>MATCHED RECORDS</span><br>"
    ...
    _download_row(
        ...
        "National summary · Bar-based status view · Zone breakdown · State coverage",
    )
    _download_row(
        "Skill Analytics",
